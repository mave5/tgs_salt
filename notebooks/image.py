...
class ImageDataGenerator(object):
    ...
    def flow(self, X, y=None, batch_size=32, shuffle=False, seed=None,
...
class NumpyArrayIterator(Iterator):
    ...
    def next(self):
        ...
        batch_y = np.zeros(tuple([current_batch_size] + list(self.X.shape)[1:]))
        if self.y is None:
            batch_y = None
        ...
        for i, j in enumerate(index_array):
            ...
            if not self.y is None:
               y = self.y[j].astype('float32')

            x, y = self.image_data_generator.random_transform(x.astype('float32'), y)
