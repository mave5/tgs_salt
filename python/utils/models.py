...
import sys
from utils.unet_resnet import ResnetBuilder,build_unet
...
def createModel(configs,showModelSummary=False):
    model = getattr(thismodule, configs.model_type)(configs.trainingParams)
    
    optimizer=configs.trainingParams["optimizer"]
    lr=configs.trainingParams["learning_rate"]
    loss=configs.trainingParams["loss"]
    ...
    if loss=='dice':
        model.compile(optimizer=optimizer, loss=customLosses.dice_coef_loss, metrics=[customLosses.dice_coef])
    elif loss=="custom":
        model.compile(loss=customLosses.loss_combined, optimizer=optimizer)
    elif loss=="averagePrecision":
        model.compile(loss=customLosses.averagePrecision_loss, optimizer=optimizer)
    elif loss=="iou_loss":
        model.compile(loss=customLosses.iou_loss, optimizer=optimizer)
        ...
        model.compile(loss=customLosses.focal_loss, optimizer=optimizer)
    else:
...
def model_UResNet34( params,**kwargs):
    h=params['h']
    w=params['w']
    z=params['z']
    input_shape=(1,128,128)
    numOfOutputs=params['numOfOutputs']
    classes=numOfOutputs
    decoder_filters=16 #params['initial_channels']
...
def model_skip5(params):
    ...
    conv12 = Conv2D(numOfOutputs, 1, data_format=data_format,kernel_regularizer=w2reg,activation=None)(conv11)
    conv12 = Conv2D(numOfOutputs, 1, data_format=data_format,kernel_regularizer=w2reg,activation=None)(conv12)
    crop1=Cropping2D(cropping=(cropping_padding,cropping_padding),data_format=data_format)(conv12)
